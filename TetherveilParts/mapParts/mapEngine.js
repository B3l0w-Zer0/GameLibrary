export function drawMap(ctx, map, player, camera, tileSize = 64) {
  ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);

  for (let y = 0; y < map.length; y++) {
    for (let x = 0; x < map[y].length; x++) {
      const tile = map[y][x];
      const { sx, sy, sw, sh } = getTileRect(tile + 12);
      ctx.drawImage(
        tileSheet,
        sx, sy, sw, sh,
        x * tileSize - camera.x,
...
export function drawPlayer(ctx, player, camX, camY, tileSize = 64) {
  ctx.fillStyle = 'red';
  ctx.beginPath();
  ctx.arc(
