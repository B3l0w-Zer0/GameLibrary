...
export async function render(container, sceneManager){
  ...
  /** @type {MapData} */
  let mapData;
  try{ mapData = await loadMap('./data/maps/startMap.json'); }
...
// Event-Listener für den Close-Button
  npcCloseBtn?.addEventListener("click", closeNPCDialog);
  ...
  window.addEventListener('keydown', e=>{
    if(e.key.toLowerCase()==='enter' && !paused && !inFight){
      ...
      if(npc){ currentNPC=npc; currentDialogIndex=0; showNPCDialog(); }
    }
  });
  ...
  conflictNo.addEventListener("click", hideKeyConflictDialog);
  ...
  async function update(){
    ...
    if(nextY>=0 && nextY<map.length && nextX>=0 && nextX<map[0].length){
      ...
      if(transfer){ mapData = await loadMap(transfer.targetMap); map=mapData.map; player.x=transfer.spawnX; player.y=transfer.spawnY; }

      if(tile===3 && (player.x!==lastPlayerPos.x || player.y!==lastPlayerPos.y)){
        if(Math.random()<0.1){ inFight=true; blackbox.style.display="flex"; console.log("Kampfbeginn!"); }
      }
    }
